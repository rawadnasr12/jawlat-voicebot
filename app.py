...
from utils.speechtotext import google_speech_to_text
...
def speechtoText(lang):
    ...
    with open(filePath+"input.ogg",'rb') as audio_file:
        ...
        print("model", model)
    #     speech_recognition_results = google_speech_to_text(audio_file)
    #     print("speech_recognition_results", speech_recognition_results)
    # return 
...
def index():
    ...
    try:
        with open('watsonassistant.json', 'r') as credentialsFile:
            credentials = json.loads(credentialsFile.read())
            global apikey, url, authenticator, assistant, assistantid, lang
    ...
    if request.method == 'POST':
        ...
        if receivedMsg == 'Ola' or receivedMsg == 'ola' or receivedMsg == 'Hi' or receivedMsg == 'hi':
            assistantid = credentials.get('en-assistant-id')
            lang = 'en'
            ...
            return str(resp)
        elif receivedMsg == 'مرحبا':
            assistantid = credentials.get('ar-assistant-id')
            lang = 'ar'
            print("assistantid", assistantid)
            session()
            message = watsonAssistant('')
